...
IR_files = sorted(get_img_file(r"/home/cuixinyu/MMIF-CDDFuse/MSRS_train/train/ir"))
VIS_files  = sorted(get_img_file(r"/home/cuixinyu/MMIF-CDDFuse/MSRS_train/train/vi"))
LABELS_files  = sorted(get_img_file(r"/home/cuixinyu/Fusion_new/datasets/MSRS/Segmentation_labels"))

assert len(IR_files) == len(VIS_files) == len(LABELS_files)
h5f = h5py.File(os.path.join('./data',
                                 data_name+'_imgsize_'+str(img_size)+"_stride_new"+str(stride)+'.h5'),
                    'w')
h5_ir = h5f.create_group('ir_patchs')
h5_vis = h5f.create_group('vis_patchs')
h5_label = h5f.create_group('labels_patchs')
train_num=0
for i in tqdm(range(len(IR_files))):
        ...
        I_LABELS = imread(LABELS_files[i]).astype(np.float32)[None, :, :]/255.  # [1, H, W] Float32)
        
        # crop
        I_IR_Patch_Group = Im2Patch(I_IR,img_size,stride)
        I_VIS_Patch_Group = Im2Patch(I_VIS, img_size, stride)  # (3, 256, 256, 12)
        I_LABELS_Patch_Group = Im2Patch(I_LABELS, img_size, stride)
        ...
        for ii in range(I_IR_Patch_Group.shape[-1]):
            ...
            bad_LABEL = is_low_contrast(I_LABELS_Patch_Group[0,:,:,ii])
            ...
            if not (bad_IR or bad_VIS ):
                ...
                avl_LABELS= I_LABELS_Patch_Group[0,:,:,ii]
                avl_IR=avl_IR[None,...]
                # avl_VIS=avl_VIS[None,...]
                avl_LABELS=avl_LABELS[None,...]

                # print(f"vi shape: {avl_VIS.shape}, ir shape: {avl_IR.shape}, label shape: {avl_LABELS.shape}")
                ...
                h5_label.create_dataset(str(train_num),    data=avl_LABELS,
                                        dtype=avl_LABELS.dtype, shape=avl_LABELS.shape)
                train_num += 1        

h5f.close()

with h5py.File(os.path.join('./data',
                                 data_name+'_imgsize_'+str(img_size)+"_stride_new"+str(stride)+'.h5'),"r") as f:
    for key in f.keys():
